...
def play(strategy0, strategy1, score0=0, score1=0, dice=six_sided,goal=GOAL_SCORE, say=silence, feral_hogs=True):
	...
	def extra_points(prev_scored, current_rolls):
		...
		prev_scored:   The points scored in the previous turn by a player
		current_rolls: The number of dice rolled at this turn
		"""
		if(abs(current_rolls - prev_scored) == 2):
			return 3
		else:
			return 0    
	...
	if(feral_hogs):
		...
		while(score0 < goal and score1 < goal):
			num_rolls = strategy0(score0, score1)        # many dice to be rolled
			turn_score = take_turn(num_rolls, score1, dice)  # save the player's score for this turn.      
			extra = extra_points(player_prev_scored, num_rolls)  # give the player extra points if deserved.
			score0 += turn_score + extra
			player_prev_scored = turn_score
			...
			num_rolls = strategy1(score1, score0)    # many dice to be rolled
			turn_score = take_turn(num_rolls, score0, dice)    # save the oponent's score for this turn.
			extra = extra_points(opponent_prev_scored, num_rolls)  # give the oponent extra points if deserved.
			score1 += turn_score + extra
