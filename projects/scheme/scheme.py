...
class Frame(object):
    ...
    def lookup(self, symbol):
        ...
        if self.bindings.get(symbol, "not found") != "not found":
    ...
    def apply(self, args, env):
        ...
        def to_list(pair):
            if pair is nil:
                ...
                return [pair.first] + to_list(pair.rest)
        python_args = to_list(args)
        if self.use_env:
        ...
        try:
            ...
            raise SchemeError("Hey! This is an error in the Class BuiltinProcedure (apply method)")
...
def do_define_form(expressions, env):
    ...
    if scheme_symbolp(target):
        ...
        lambda_fn = do_lambda_form(Pair(target.rest, expressions.rest), env)
        env.define(identifier, lambda_fn)
        return identifier
    else:
